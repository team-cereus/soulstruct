...
class SoulstructConfig:
    ...
    NIGHTREIGN_PATH: Path = field(
        default_factory=lambda: _DEFAULT_STEAM_PATH / "ELDEN RING NIGHTREIGN/Game"
    )
    ...
    def load(self) -> None:
        ...
        if "SEKIRO_PATH" in data and "SEK_PATH" not in data:
            data["SEK_PATH"] = data.pop("SEKIRO_PATH")
...
NIGHTREIGN_PATH: Path = Config.NIGHTREIGN_PATH
SEKIRO_PATH: Path = Config.SEK_PATH
ELDEN_RING_PATH: Path = Config.ER_PATH
